...
export function DashboardProof({ audience }: { audience: "business" | "creator" }) {
  ...
  const tabs = isBusiness
    ...
    : [t("할 일", "To do"), t("성과", "Performance"), t("탐색", "Discover"), t("제출 이력", "Submission history")];
  const rows = isBusiness
    ? [
        { title: t("캠페인 브리프 작성", "Create a campaign brief"), meta: t("목표·작업 범위·모집 조건을 단계별로 설정", "Set goals, scope, and recruiting conditions step by step"), status: t("다음 작업", "Next") },
    ...
    : [
        ...
        { title: t("새 캠페인 탐색", "Discover a new campaign"), meta: t("브랜드·작업 범위·마감 조건부터 비교", "Compare brand, scope, and deadline first"), status: t("탐색", "Browse") },
      ];
  ...
  return (
    <div className={styles.dashboardProof} aria-label={t(`${isBusiness ? "브랜드" : "크리에이터"} 대시보드 제품 화면 예시`, `${isBusiness ? "Brand" : "Creator"} dashboard product preview`)}>
      ...
      <div className={styles.proofMetricStrip} aria-hidden="true">
        {(isBusiness
          ? [t("전체 캠페인", "Campaigns"), t("모집 중", "Recruiting"), t("누적 지원", "Applications"), t("검수 대기", "In review")]
          : [t("진행 작업", "Active work"), t("검수 대기", "In review"), t("완료", "Completed"), t("제출 이력", "Submissions")]
        ).map((label) => (
          <span key={label}><small>{label}</small><strong>—</strong></span>
        ))}
      </div>
      ...
      <div className={styles.proofWorkspace}>
        ...
        <div className={styles.proofSide}>
          ...
          <div className={styles.proofNotice}>
            ...
            <span><strong>{t("검수 이력 연결", "Review history stays connected")}</strong><small>{t("수정 사유와 제출 회차를 한 캠페인에서 확인합니다.", "Review reasons and submission takes stay in one campaign.")}</small></span>
          </div>
